...
import realtimebidding.models.{BidResponse, Error, NotExist, RestMessage, SuccessfulOperation}
import realtimebidding.routing.PerRequestActor.WithProps
...
trait PerRequestActor extends Actor with JsonSupport {
  ...
  override def receive: Receive = {
    case res: BidResponse => complete(OK, res)
    case SuccessfulOperation => complete(OK, EmptyJson)
    case ne: NotExist => complete(NoContent, ne)
